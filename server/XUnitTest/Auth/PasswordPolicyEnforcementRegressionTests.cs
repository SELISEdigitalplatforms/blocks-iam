...
public class PasswordPolicyEnforcementRegressionTests
{
    ...
    [InlineData("^ABC$", "abc", true)]
    [InlineData(@"^(?=.*\d).{10,64}$", "Abcdefg1", false)]
    public async Task TheTenantRegexIsTheOneAuthority(string? regex, string password, bool expected)
    {
        // The screens describe this same regex, decoded, so what a user is shown and what is
        // enforced here cannot drift into two different rules.
        var config = new Mock<IIamConfigurationRepository>();
