...
public static class CaptchaDriverServiceExtension
{
    public static void RegisterBlocksCaptchaService(this IServiceCollection services)
    {
        ...
        // Verification handlers - all three are resolved by name via CaptchaVerificationServiceProvider
        // (provider defaults to "bcaptcha"), so every concrete handler must be registered.
